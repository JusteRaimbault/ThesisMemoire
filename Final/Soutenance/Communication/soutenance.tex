...
% Review Florent
%. X slide 2 : titre et texte non explicites ; l'info réellement important n'est pas écrite, par ex. "manifestations concrètes" de quoi? > il faut que l'info soit hiérarchisée, et simple (tes trois lignes sont assez complexes à comprendre et à mon sens, trop)
%  X  slide 3 : enlever "naturelle" ; les schémas à droite, pourquoi? en garder 1 seul, signifiant? 
%. -> les scehma montrent progression
%  X slide 5 : tu t'évertues avec cette carte en chinois. vraiment, je pense que ce n'est pas un bon choix. ni scientifiquement, ni pédagogiquement, ni en termes d'image que tu renvoies (pour moi cela dit : je parles une langue que vous ne parlez pas mais c'est pas grave pour moi : je comprends) ; par ailleurs pourquoi est-ce "théorique"? c'est plutôt empirique je dirais? globalement pas utile les illustrations ; plutôt dire à quoi s'appliquent ces domaines de connaissance?
% progression 5, 6, 7 : difficile, surtout qu'il manque un 4bis qui justifie concrètement le besoin de rentrer par les domaines de connaissance. tu n'as pas vraiment pris en compte notre discussion à cet égard, c'est dommage ; tu devrais d'abord dire pourquoi, concrètement, c'est dur de définir de caractériser et de modéliser la coévolution, ce qui existe avant toi
% slide 8 : perspectives théoriques :titre maladroit car à ce stade trop tôt pour tirer des perspectives
...
\sframe{Une approche originale}{

% Un ancrage progressif dans la complexité
%Formation scientifique multidisciplinaire
% 			\item Objet ville: architecture, unbanisme.
% \item Ville, Environnement, Transport % Preciser Ponts

\begin{columns}
	\begin{column}{0.6\textwidth}
		\textit{Parcours personnel}
		\medskip
		\begin{itemize}
			\item Ingénieur généraliste
			\item Intérêt pour l'objet ville : expériences en architecture et urbanisme, puis formation aux Ponts et Chaussées
			\item Une transition progressive vers les sciences humaines et la géographie, tout en gardant un ancrage fort dans les systèmes complexes
		\end{itemize}
		\medskip
		
		$\rightarrow$ \textit{Une articulation théorique et thématique structurante.}
...
% Du devenir de l'accessibilité à Tangjia
...
Des dynamiques \textit{co-évolutives} entre réseaux de transport et territories suggérées par de nombreux travaux (Théorie Evolutive des Villes)

\bigskip

\textbf{Axe 1 : } \textit{Proposition d'une définition et d'une méthode de caractérisation empirique des ces dynamiques co-évolutives.}

\bigskip

Connaissance limitée par les seules études empiriques (données pauvres, cas d'étude, temps long, couplage fort): utilisation de la modélisation comme outil de connaissance.

\bigskip

\textbf{Axe 2 : } \textit{Construction de modèles de co-évolution des réseaux de transport et des territoires.}
...
%  definition des objets et de la co-evolution
%  pourquoi théorie evolutive et morphogenese
...
\sframe{Entrée théorique : définitions}{
  ...
  \begin{itemize}
  	\item Villes et territoires lus au prisme de la \textit{Théorie Evolutive des Villes}% (Pumain, 1997)
  	\item Réseaux de transport comme matérialisation de ``projets transactionnels'', suivant la \textit{Théorie Territoriale des Réseaux} %(Dupuy, 1987)
  \end{itemize}
...
\begin{enumerate}
	\item niveau individuel d'individus donnés
	\item niveau statistique des populations d'individus%, évoluant dans des \textit{niches}
	\item niveau global du système
\end{enumerate}  
...
\begin{enumerate}
	\item Une entrée par la morphogenèse correspond à la notion de niche et l'échelle mésoscopique
	\item Une entrée systémique par la théorie évolutive à l'échelle macroscopique
\end{enumerate}
...
%\begin{column}{0.35\textwidth}
	...
	\textit{Effets de réseau robustes au suraprentissage révélés par le modèle avec réseau statique}
...
\sframe{Modèles macroscopiques : co-évolution}{
...
\textit{Un modèle de morphogenèse par réaction-diffusion et multi-modélisation de la croissance du réseau}

\medskip

{\centering
\includegraphics[width=0.3\linewidth]{figures/meso-nwgrowth.png}
\includegraphics[width=0.7\linewidth]{figures/meso-calib.jpg}
}
...
\textit{Le modèle Lutecia : vers une intégration des processus de gouvernance}
...
%%%%%
%% Slide domaine connaissance
%%%%%


%% 3
%  Cette présentation est egalement l'occasion de suggérer un point de vue original sur le travail effectué. Nous proposons ainsi une entrée non pas linéaire, mais par les différents domaines de connaissance, 
...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\node (b) at (-1,0) {\includegraphics[width=2cm]{figures/domconn-empirical.png}};
			\node (c) at (3,5) {\includegraphics[width=3cm]{figures/domconn-schema.pdf}};
			\node (d) at (3,3.5) {Théorique};
			\node (e) at (7,1.5) {Modélisation};
			\node (f) at (7,0) {\includegraphics[width=3cm]{figures/domconn-modeling.png}};
			\draw[<->] (a) -- (d);
			\draw[<->] (d) -- (e);
			\draw[<->] (a) -- (e);
		\only<2->{
			\node (g) at (-1,5) {\textit{Méthodes}};
			\node (h) at (-1,4) {\includegraphics[width=3cm]{figures/domconn-methods.pdf}};
			\node (i) at (7,5) {\textit{Outils}};
			\node (j) at (7,3.8) {\includegraphics[width=3cm]{figures/domconn-tools.pdf}};
			\node (k) at (3,1) {\textit{Données}};
			\node (l) at (3,-0.5) {\includegraphics[width=3cm]{figures/domconn-data.pdf}};
		}
	\end{tikzpicture}
...
\begin{itemize}
	...
	\item Multi-modélisation de la co-évolution à l'échelle macroscopique
\end{itemize}
...
\begin{itemize}
	...
	\item \textit{Vers des théories intégrées des systèmes territoriaux : } modèles multi-échelle et couplage de la théorie évolutive avec le \textit{Scaling}, réflexivité et épistémologie, intelligence artificielle.
\end{itemize}
